    ...
    <div className={dark ? 'app-shell dark' : 'app-shell'}>
      ...
      <main className="workspace">
        ...
        <section className="chat-area">
          ...
          {tool === 'translate' && <div className="tool-card"><div className="tool-card-header"><div className="tool-icon"><FileText /></div><div><h2>Translate a PDF</h2><p>Keep structure, change the language.</p></div></div><label className="upload-zone"><Upload /><strong>{pdfName || 'Drop a PDF here'}</strong><small>or choose a file · up to 25 MB</small><input type="file" accept=".pdf,application/pdf" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; if (file.size > 10 * 1024 * 1024) { setUploadError('PDF must be smaller than 10 MB.'); return } setPdfName(file.name); setUploadError(''); setAttachments([file]) }} /></label><div className="language-row"><label>From<select><option>Auto-detect</option><option>English</option><option>বাংলা</option></select></label><span>→</span><label>To<select><option>বাংলা</option><option>English</option><option>Español</option></select></label></div><button className="primary-button" disabled={!pdfName} onClick={() => setTranslated(true)}>{translated ? 'Translation ready' : 'Translate PDF'}<Languages /></button></div>}
          <div className="composer-wrap">{uploadError && <p className="upload-error" role="alert">{uploadError}</p>}{attachments.length > 0 && <div className="attachment-list" aria-label="Attached files"><span className="attachment-summary">{attachments.length} file{attachments.length > 1 ? 's' : ''} attached</span>{attachments.map((file) => <button type="button" key={`${file.name}-${file.lastModified}`} className="attachment-chip" title={file.name} onClick={() => setAttachments((current) => current.filter((item) => item !== file))}>{file.type.startsWith('image/') ? 'Image' : file.type.startsWith('audio/') ? 'Audio' : 'PDF'} ×</button>)}</div>}<div className="composer"><label className="composer-icon" aria-label="Attach image, audio, or PDF"><Paperclip /><input className="attachment-input" type="file" accept=".pdf,application/pdf,image/*,audio/*" multiple onChange={(event) => { const files = Array.from(event.target.files || []); const valid = files.filter((file) => file.size <= 10 * 1024 * 1024); setUploadError(valid.length < files.length ? 'Each file must be smaller than 10 MB.' : ''); setAttachments(valid.slice(0, 3)); event.currentTarget.value = '' }} /></label><textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing && event.keyCode !== 229) { event.preventDefault(); send() } }} placeholder={tool === 'translate' ? 'Add a PDF above to begin…' : `Message ${assistants[selected][0]}…`} rows={1} /><button className="send-button" onClick={() => send()} aria-label="Send message"><ArrowUp /></button></div><p className="composer-note">Boithok can make mistakes. Check important information.</p></div>
        </section>
      </main>
    </div>
  )
