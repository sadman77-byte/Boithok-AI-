...
function realtimeContext() {
  const now = new Date()
  const parts = (calendar: string, locale: string) => new Intl.DateTimeFormat(locale, { calendar, year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' }).formatToParts(now).reduce<Record<string, string>>((result, part) => { result[part.type] = part.value; return result }, {})
  const gregorian = parts('gregory', 'en-GB')
  const bengali = parts('beng', 'bn-BD')
  return `Current UTC date/time: ${now.toISOString()}. Format calendar answers exactly as requested: Bengali: "${bengali.day} ${bengali.month}, ${bengali.year}" (example: "৬ ভাদ্র, ১৪৩৩"); English/Gregorian: "${gregorian.day} ${gregorian.month}, ${gregorian.year}" (example: "21 August, 2026"). Include the calendar name only when useful. Do not output locale formats like "ভাদ্র ৬" or "August 21" when the user asks for this style. Other calendar values are available from Intl.DateTimeFormat if requested.`
}
